...
def scan_namespace_health(k8s_adapter, ns):
    ...
    for name in pod_names:
        ...
        if diag.get("status") == "SUCCESS":
            ...
            # --- CRITÉRIO CIRÚRGICO DE SAÚDE (ESTADO ATUAL) ---
            # Restart antigo não significa falha atual.
            # Após reiniciar o Minikube, pods podem ficar Running/Ready com restart_count > 0.
            ...
            if container_states:
                for c in container_states:
                    ...
                    # Se o container não está pronto agora, ele não está saudável.
                    if not ready:
                        containers_ok = False
                        falha_detectada = reason or state or "ContainerNotReady"
                        break

                    # Só considera falha se houver problema ativo no estado atual.
                    # Não considera restart_count antigo como falha.
                    if state == "waiting" and reason in active_bad_reasons:
                        containers_ok = False
                        falha_detectada = reason
                        break

                    if state == "terminated":
                        if exit_code not in (None, 0, "0"):
                            containers_ok = False
                            falha_detectada = reason or f"Terminated(exitCode={exit_code})"
                            break
            else:
                containers_ok = False
                falha_detectada = "SemContainerStatus"

            # Eventos antigos de probe não devem derrubar um pod que está Running e Ready.
            has_probe_failure = False
            if not (phase_ok and containers_ok):
                has_probe_failure = any(
                    "probe" in str(iss.get("message", "")).lower()
                    for iss in issues
